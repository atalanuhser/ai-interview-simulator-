...
def start_interview(candidate_name: str, position: str, job_text: str, cv_text: str):
    ...
    response = chat.send_message("Mülakata başla. İlk soruyu sor.")
    return chat, response.text.strip()


def send_answer(chat, user_answer: str):
    """
    Adayın cevabını gönderir, sonraki soruyu veya bitiş sinyalini döner.
    Döner: (sonraki_soru: str, bitti_mi: bool, final_data: dict | None)
    """
    response = chat.send_message(user_answer)
    text = response.text.strip()
    clean = text.replace("```json", "").replace("```", "").strip()

    if '"mülakat_bitti": true' in clean:
        try:
...
def get_chat_history(chat) -> list:
    """
    Chat geçmişini soru-cevap çiftleri olarak döner.
    Yedek puanlama için kullanılır.
    """
    history = []
